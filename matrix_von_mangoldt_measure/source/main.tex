...
cutoff, which is retained and varied. Differentiating the
finite matrix path $u\mapsto Q_N(u)$ across a prime-power threshold $u=\log q$
returns the von Mangoldt weight exactly, in every matrix entry: the
first-derivative jump is
\[
        Q_N'(\log q+0)-Q_N'(\log q-0)
        =-\frac{2\Lam(q)}{\sqrt q\,\log q}\,\one_N\one_N^t .
\]
Equivalently, the singular part of $D_u^2Q_N$ is a negative semidefinite
rank-one \emph{matrix-valued von Mangoldt measure}, a finite realization of the
prime side of the Weil--Guinand explicit formula (with no archimedean cutoff) on
the operator path itself; the identity relating this prime side to the zeros is
classical and is not proven here. We prove that this event is
\emph{arithmetically rigid}:
...
more. The event geometry is otherwise a piece of finite harmonic analysis: the
...
sharp finite \emph{prime-edge uncertainty principle} bounds the order at which a
...
the velocity spectrum. Every statement is finite and exact; the event algebra
holds verbatim for any $L$-function of the Selberg class (the arithmetic entering
only through the source weights), while the sign-definite consequences require a
positive weight and so hold for $\zeta$. The results are structural; the paper
...
approximate the zeros (Paper~1). In this sense the finite path renders the
...
payoff: averaging its $(2N+1)^2$ entries reads $\Lam(q)$ with variance reduced by
the same factor. What remains is finite algebra of independent interest. The
finite source-to-jet map is a lossless dictionary whose nodes are the squares
$0,1^{2},\dots,N^{2}$ (Section~\ref{sec:dictionary}); a sharp uncertainty
principle governs how blind a band-limited probe can be to an entering prime,
...
algebra is universal across the Selberg class, with a residue-class readout of
$\Lam(q)$ across the Dirichlet family; the sign-definite parts require a positive
...
squared-node event dictionary and the finite prime-edge uncertainty principle,
...
spectral-error regime \cite{Sliwinski2026}; Silva records related Loewner,
exact-entry, and operator-monotone framings
\cite{Silva20710075,Silva20737111,Silva20671635}; and Liao uses prime-event
...
inequality \cite{Daubechies1992} are used as tools; the novelty is their exact
...
exactly the confluent limit $g_\omega'(m)$. The whole event package below uses
only this structure. Indeed, for any real-analytic background, amplitude
$s(u)$, and reduced position $\omega(u)$ with $\omega(u_0)=0$,
$\omega'(u_0)\ne0$, and $\tfrac{d}{d\omega}g_\omega|_0$ affine on the nodes with
nonzero slope $\lambda$, the path $R(u)-s(u)\mathrm{DD}_X[g_{\omega(u)}]$ has the
...
finite \cvs\ path is the case $g_\omega=\sin(2\pi x\omega)/\pi$ ($\lambda=2$),
...
Paper~2 \cite[the finite archimedean and pole blocks]{Groskin2026FiniteDictionary}:
...
\begin{lemma}[Background regularity]\label{lem:background}
...
the $(2N+1)^2$ entries are $(2N+1)^2$ independent measurements of the same scalar,
and averaging them is optimal.
...
entrywise errors $E_{mn}$ of mean zero and common variance $\sigma^2$. The
minimum-variance linear unbiased estimator of $a_q$ is the coincidence average
\[
...
orthogonal to $\one_N\one_N^t$, and its size, equivalently the ratio
$\sigma_2/\sigma_1$ of the two leading singular values of $\widehat{J}$, certifies
the rank-one model.
\end{corollary}
...
average, and for uncorrelated entries
...
\section{Arithmetic rigidity}\label{sec:rigidity}
...
the map from the family of singular jets to $\Lam(q)$ is a bijection, and no
...
quotient has dimension exactly $2N+1$.
\end{lemma}
...
\section{A finite prime-edge uncertainty principle}\label{sec:uncertainty}
...
variable $\tau$; we call $2e(x)+1$ the \emph{visibility order} of $x$.

\begin{theorem}[Finite prime-edge uncertainty ceiling]\label{thm:uncertainty}
...
ceiling. The principle is sharp and the extremizer unique: a level-$N$
...
\caption{The finite prime-edge uncertainty ceiling at $N=4$. Left: the unique
...
$0<\omega\le\omega_*(x)$ (any $\omega_*$ below the first positive root of the
remainder, whose scale is set by the ratio of successive even moments of $x$);
...
\]
a rational function with a simple pole of residue $-|\langle\phi_j,\one_N\rangle|^2$
at each eigenvalue $\lambda_j$ of $Q_0$. Then, at $u_0$, for $z\notin
...
$(D_u^2\operatorname{tr}Q_N)_{\rm sing}=-d\sum_q a_q\delta_{\log q}$: its second
derivative is exactly $-(2N+1)$ times the prime side of the explicit formula, a
scalar reading of Corollary~\ref{cor:measure}.
\end{theorem}
...
the couplings to the universal direction, and scaled by $-a_q$ they are exactly
...
\begin{theorem}[Elementary spectral barriers in a positive cell]\label{thm:barriers}
Suppose $Q_0=Q_N(u_0)$ is positive definite. Let $e_r(Q)$ be the $r$th elementary
...
decelerate at every prime event.
\end{theorem}
...
\subsection*{The prime event as a Krein-string boundary mass}

The rank-one velocity deflation has a clean reading in the classical language of
Krein strings and de Branges canonical systems. Let $V_\pm=Q_N'(u_0\pm0)$ be the
...
\]
$W_\pm$ is a rational Nevanlinna function, the
Weyl--Titchmarsh coefficient of the finite Krein string obtained by
tridiagonalizing $V_\pm$ from $\one_N$ (a genuine finite string when $\one_N$ is
cyclic; the identity below needs no such assumption).

\begin{theorem}[Prime event as a boundary-mass increment]\label{thm:krein}
At every $z$ off the spectra of $V_\pm$,
\[
        \frac{1}{W_+(z)}=\frac{1}{W_-(z)}-a_q,
        \qquad a_q=\frac{2\Lam(q)}{\sqrt q\,\log q}>0,
\]
a jump \emph{independent of $z$}. Equivalently $V_+$ is the Schur complement, with
...
(2\Lam(q))$. In the Krein correspondence this appends one boundary point mass
$a_q$ to the coincidence string. Consequently the von Mangoldt weight is that
boundary mass,
\[
...
\begin{proof}
Apply the Sherman--Morrison identity to $V_+=V_--a_q\one_N\one_N^t$: with
$M=V_--z$, $W_+=\one_N^t(M-a_q\one_N\one_N^t)^{-1}\one_N=W_-/(1-a_qW_-)$, hence
$1/W_+=1/W_--a_q$. The Schur statement is the identity
$V_--\one_N\,a_q\,\one_N^t=V_+$ for the displayed border; adding a constant to
$-1/W$ is the boundary point-mass move in Krein's string--to--continued-fraction
correspondence.
\end{proof}

\begin{remark}
This places the finite \cvs\ prime event in the same de Branges--Krein
canonical-system setting as Suzuki's continuous screw-function pencil
\cite{Suzuki2026}: the prime data his continuous screw function carries appears
here, under the reduced parametrization ($\omega_q'(u_0)=1/\log q$), as the finite
per-event boundary mass $a_q=2\Lam(q)/(\sqrt q\,\log q)$. Only the \emph{per-event} boundary
increment equals $a_q$; the interior masses of the string encode the accumulated
spectral history of $V_-$, not the individual primes, and nothing here bears on
positivity of the finite form or on the zeros.
\end{remark}

\section{Universality, the explicit formula, and scope}\label{sec:scope}

\paragraph{Universality across the Selberg class.}
The prime block is determined only by the source amplitudes and the
...
term. For an $L$-function $L(s,\pi)$ with prime side $\sum_q c_\pi(q)q^{-1/2}$
(Dirichlet $c_\chi(q)=\Lam(q)\chi(q)$, or automorphic $c_\pi(q)=\Lam(q)a_\pi(q)$),
the finite path has first-derivative jump
...
uncertainty ceiling depend only on $A_N$ and hold for arbitrary (in particular
...
barrier decrease (Theorem~\ref{thm:barriers}) all use $a_q>0$, hence a
\emph{positive} weight $c_\pi(q)>0$. This holds at every prime power for $\zeta$
(where $c(q)=\Lam(q)>0$), so those results transfer verbatim there. For a real
...
Fix a prime-power edge $u_0=\log q$ with $(q,m)=1$, and for each Dirichlet
character $\chi\bmod m$ let $J^{(\chi)}=j^{(\chi)}\one_N\one_N^t$ be the
first-derivative jump of the finite path built from $L(s,\chi)$, so
$j^{(\chi)}=-2\Lam(q)\chi(q)(\log q)^{-1}q^{-1/2}$. Then for every residue $a$ with
$(a,m)=1$,
\[
...
approximation of Paper~1 \cite{Groskin2026Paper1}. The object is thus
...
exactly $\Lam(q)$, and the reconstruction, uncertainty, and deflation results are
...
The proofs are algebraic; the companion scripts are reproducibility guards for
signs, indexing, determinant factors, and finite-field behavior, not proof
substitutes. The workspace records, over exact integer or modular arithmetic:
\[
\begin{array}{ll}
\text{rank-one jump / first-jump end-to-end} & q\in\{3,4,5,7,8,9,25\},\ N\le6,\\
\text{event-jet rank and determinant} & N\le200\ \text{over four prime fields},\\
\text{recurrence} & N\le1000\ \text{over four prime fields},\\
\text{event dictionary (window, blind line)} & N\le200,\\
\text{uncertainty ceiling (unique extremizer)} & N\le10,\\
\text{spectral-barrier deceleration} & 50\ \text{positive definite integer cases},\\
\text{universal jet }B_{r,N}\text{ closed form} & \text{exact symbolic, orders }r\le5,\\
\text{coincidence-averaged readout} & (2N+1)^2\ \text{variance reduction},\\
\text{Dirichlet residue-class readout} & \text{cyclic moduli }m\le13.
\end{array}
\]
...
recovered from a single entry to $5\times10^{-8}$. The archimedean and pole
...
The verification package (the reproducibility guard scripts, their exact JSON
